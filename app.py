...
def init_session():
    ...
    if 'id_created' not in st.session_state:
        st.session_state.id_created = False
...
def show_welcome_screen():
    ...
    with col1:
        st.image(light_theme, caption="Light Theme ID Card", use_column_width=True)
    
    with col2:
        st.image(dark_theme, caption="Dark Theme ID Card", use_column_width=True)
...
def show_id_card_screen():
    ...
    if st.session_state.id_created and st.session_state.id_card_image:
        st.image(st.session_state.id_card_image, caption="Your ClaudeNation ID")
    ...
    # If returning user, regenerate ID from stored data
        ...
        if citizen_data:
            ...
            if photo and st.button("Generate ID Card"):
                ...
                st.image(id_card, caption="Your regenerated ClaudeNation ID")
